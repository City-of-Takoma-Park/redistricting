...
## new maps for new options

wards_upd <- st_read("./data/new_options/TP_Census_Blocks_2020_w_Options.gdb", layer = "Takoma_Park_Census_Blocks_w_Revised_Ward_Options_AandB") %>%
  rename_all(tolower) %>%
  st_transform(4326)

# st_layers("./data/new_options/TP_Census_Blocks_2020_w_Options.gdb")
...
new_ward_funct <- function(df, opt_num){
  ...
  df %>%
    mutate(!!sym(ward_chng) := !!sym(ward) == current_ward)
...
leaflet_wards <- function(basemap, df, opt_num, grp = ": changed blocks", curr_ward = F){
  ...
  addpoly_legend(basemap_select = basemap, df_select = df %>% st_drop_geometry(), pal_funct_select = colors_wards, variable_select = ifelse(curr_ward, "current_ward", paste0("option_", opt_num)), group_select =ifelse(curr_ward, "Current wards", paste0("Option ", opt_num, grp)), title_select = ifelse(curr_ward, "Current wards", paste0("Option ", opt_num, grp)), labels_select = label_funct(df %>% st_drop_geometry(), opt_num), .data = df)
}

grp_vac <- c(
  "Current wards",
  map_chr(c("a", "b"), ~ paste0("Option ", .x)),
  map_chr(c("a", "b"), ~ paste0("Option ", .x, ": changed blocks"))
)

chng_map <- leaflet(wards_upd) %>%
  addTiles(options = tileOptions(opacity = 0.4)) %>%
  leaflet_wards(wards_upd, "", curr_ward = T,grp = "") %>%
  leaflet_wards(wards_upd, "a", curr_ward = F, grp = "") %>%
  leaflet_wards(wards_upd, "b", curr_ward = F, grp = "") %>%
  leaflet_wards(wards_upd, "a", curr_ward = F) %>%
  leaflet_wards(wards_upd, "b", curr_ward = F) %>%
  addLayersControl(overlayGroups = grp_vac, position = "topleft", options = layersControlOptions(collapsed = F)) %>%
  hideGroup(group = grp_vac[-1]) %>%
  add_wards()
...
htmlwidgets::saveWidget(chng_map, file = "./updated-maps.html")
htmlwidgets::saveWidget(carto_map, file = "./new-maps.html")
htmlwidgets::saveWidget(esri_map, file = "./maps-updated.html")
